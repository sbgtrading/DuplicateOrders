...
public enum KimM4movingaverages_MsgLengthType {Verbose, Terse}
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	public class KimM4movingaverages : Indicator
	{
//====================================================================================================
//====================================================================================================
...
//====================================================================================================
		...
		private KimM4movingaverages_type pMA1Type = KimM4movingaverages_type.EMA;
		private KimM4movingaverages_type pMA2Type = KimM4movingaverages_type.EMA;
		private KimM4movingaverages_DataType pMA1DataType = KimM4movingaverages_DataType.Close;
		private KimM4movingaverages_DataType pMA2DataType = KimM4movingaverages_DataType.Close;
		...
		protected override void OnStateChange(){
			...
			{
				bool IsBen = NinjaTrader.Cbi.License.MachineId.CompareTo("CB15E08BE30BC80628CFF6010471FA2A")==0 && System.IO.File.Exists("c:\\222222222222.txt");
				if(!IsBen)
...
//==================================================================
		protected override void OnBarUpdate()
		{
			...
			if(RunInit)
			{
				...
				if(pMA1DataType == KimM4movingaverages_DataType.Open)    x = Open;
				if(pMA1DataType == KimM4movingaverages_DataType.High)    x = High;
				if(pMA1DataType == KimM4movingaverages_DataType.Low)     x = Low;
				if(pMA1DataType == KimM4movingaverages_DataType.Median)  x = Median;
				if(pMA1DataType == KimM4movingaverages_DataType.Typical) x = Typical;

				if(pMA1Type == KimM4movingaverages_type.EMA)       TheMA1 = EMA(x,pPeriod1);
				else if(pMA1Type == KimM4movingaverages_type.SMA)  TheMA1 = SMA(x,pPeriod1);
				else if(pMA1Type == KimM4movingaverages_type.HMA)  TheMA1 = HMA(x,pPeriod1);
				else if(pMA1Type == KimM4movingaverages_type.TMA)  TheMA1 = TMA(x,pPeriod1);
				else if(pMA1Type == KimM4movingaverages_type.TEMA) TheMA1 = TEMA(x,pPeriod1);
				else if(pMA1Type == KimM4movingaverages_type.WMA)  TheMA1 = WMA(x,pPeriod1);
				else if(pMA1Type == KimM4movingaverages_type.LinReg)        TheMA1 = LinReg(x, pPeriod1);
				else if(pMA1Type == KimM4movingaverages_type.ZeroLagEMA)    TheMA1 = ZLEMA(x, pPeriod1);
				else if(pMA1Type == KimM4movingaverages_type.ZeroLagHATEMA) TheMA1 = IW_ZeroLagHATEMA(x, pPeriod1);
				else if(pMA1Type == KimM4movingaverages_type.ZeroLagTEMA)   TheMA2 = IW_ZeroLagTEMA(x, pPeriod1, -1);
				else if(pMA1Type == KimM4movingaverages_type.KAMA)       TheMA1 = KAMA(x, pKAMAfast1, pPeriod1, pKAMAslow1);
				else if(pMA1Type == KimM4movingaverages_type.VWMA)       TheMA1 = VWMA(x, pPeriod1);
				else if(pMA1Type == KimM4movingaverages_type.VMA)        TheMA1 = VMA(x, pPeriod1, pFVolatilityPeriod);

				if(pMA2DataType == KimM4movingaverages_DataType.Open)    x = Open;
				if(pMA2DataType == KimM4movingaverages_DataType.High)    x = High;
				if(pMA2DataType == KimM4movingaverages_DataType.Low)     x = Low;
				if(pMA2DataType == KimM4movingaverages_DataType.Median)  x = Median;
				if(pMA2DataType == KimM4movingaverages_DataType.Typical) x = Typical;
//				switch (pMA2Type)
//				{	case KimM4movingaverages_type.SMA  : TheMA2 = SMA(x,pPeriod2); break;
//					case KimM4movingaverages_type.EMA  : TheMA2 = EMA(x,pPeriod2); break;
//					case KimM4movingaverages_type.WMA  : TheMA2 = WMA(x,pPeriod2); break;
//					case KimM4movingaverages_type.HMA  : TheMA2 = HMA(x,pPeriod2); break;
//					case KimM4movingaverages_type.TEMA : TheMA2 = TEMA(x,pPeriod2); break;
//					case KimM4movingaverages_type.VWMA : TheMA2 = VWMA(x,pPeriod2); break;
//					case KimM4movingaverages_type.TSF  : TheMA2 = TSF(x,1,pPeriod2); break;
//					case KimM4movingaverages_type.LinReg  : TheMA2 = LinReg(pPeriod2); break;
//				}
				if(pMA2Type == KimM4movingaverages_type.EMA)       TheMA2 = EMA(x,pPeriod2);
				else if(pMA2Type == KimM4movingaverages_type.SMA)  TheMA2 = SMA(x,pPeriod2);
				else if(pMA2Type == KimM4movingaverages_type.HMA)  TheMA2 = HMA(x,pPeriod2);
				else if(pMA2Type == KimM4movingaverages_type.TMA)  TheMA2 = TMA(x,pPeriod2);
				else if(pMA2Type == KimM4movingaverages_type.TEMA) TheMA2 = TEMA(x,pPeriod2);
				else if(pMA2Type == KimM4movingaverages_type.WMA)  TheMA2 = WMA(x,pPeriod2);
				else if(pMA2Type == KimM4movingaverages_type.LinReg)        TheMA2 = LinReg(x, pPeriod2);
				else if(pMA2Type == KimM4movingaverages_type.ZeroLagEMA)    TheMA2 = ZLEMA(x, pPeriod2);
				else if(pMA2Type == KimM4movingaverages_type.ZeroLagHATEMA) TheMA2 = IW_ZeroLagHATEMA(x, pPeriod2);
				else if(pMA2Type == KimM4movingaverages_type.ZeroLagTEMA)   TheMA2 = IW_ZeroLagTEMA(x, pPeriod2, -1);
				else if(pMA2Type == KimM4movingaverages_type.KAMA)       TheMA2 = KAMA(x, pKAMAfast2, pPeriod2, pKAMAslow2);
				else if(pMA2Type == KimM4movingaverages_type.VWMA)       TheMA2 = VWMA(x, pPeriod2);
				else if(pMA2Type == KimM4movingaverages_type.VMA)        TheMA2 = VMA(x, pPeriod2, pFVolatilityPeriod);
				#endregion
			}
...
//}
			...
			BackBrush = null;
			Signal[0] = (FLAT);
			string tag = string.Format("KimM4movingaverages_{0}",CurrentBar);
...
//			}
			...
			if(pShowInwardSignals || zone==0) {
				...
				if(CrossUp) {	
					...
					PlaySound(AddSoundFolder(AddSoundFolder(pSpeakOnUpwardCross));

//					if(zone==0){
//						msg = string.Format("MAC is LONG on {0}",Bars.BarsPeriod.ToString());
//						if(pMsgLength==KimM4movingaverages_MsgLengthType.Verbose)
...
//						PlaySound(AddSoundFolder(pSoundFileInwardDown));
					...
					if(EmailsThisBar<pMaxEmails && emailaddress.Length>0) {
						EmailsThisBar++;
						msg = string.Format("KimM4movingaverages", Instrument.ToString(), Bars.BarsPeriod.ToString());
						Subj = msg;
...
//						PlaySound(AddSoundFolder(pSoundFileOutwardUp));
					...
					PlaySound(AddSoundFolder(pSpeakOnDownwardCross));
...
//						PlaySound(AddSoundFolder(pSoundFileOutwardDown));
			...
			if(Open[0] <= TheMA1[0] && Close[0] > TheMA1[0]){
				...
				if(AlertsThisBar<pMaxAlerts && pSoundFileStraddleOverMA1.Length>0) {
					msg = string.Format("KimM4movingaverages",Bars.BarsPeriod.ToString());
...
//					PlaySound(AddSoundFolder(pSoundFileBarUnderMA2));
			...
			if(!pShowMALines) {
				...
				if(pColoringBasis == KimM4movingaverages_ColoringBasis.OnCrossing) {
				...
				} else if(pColoringBasis == KimM4movingaverages_ColoringBasis.OnTrendChange) {
				...
				} else if(pColoringBasis == KimM4movingaverages_ColoringBasis.NoColorChange) {
					PlotBrushes[0][0] = pLineUpColorFast;
					PlotBrushes[1][0] = pLineUpColorSlow;
				}
			}
		...
		public override string ToString(){
			...
			if(MA1Type==KimM4movingaverages_type.VMA)  type1 = "VMA("+this.pPeriod1.ToString()+","+this.pFVolatilityPeriod.ToString()+")";
			if(MA1Type==KimM4movingaverages_type.KAMA) type1 = "KAMA("+this.pKAMAfast1.ToString()+","+this.pPeriod1.ToString()+","+this.pKAMAslow1.ToString()+")";
			string type2 = MA2Type.ToString()+"("+pPeriod2.ToString()+")";
			if(MA2Type==KimM4movingaverages_type.VMA)  type2 = "VMA("+this.pPeriod2.ToString()+","+this.pSVolatilityPeriod.ToString()+")";
			if(MA2Type==KimM4movingaverages_type.KAMA) type2 = "KAMA("+this.pKAMAfast2.ToString()+","+this.pPeriod2.ToString()+","+this.pKAMAslow2.ToString()+")";
...
//		private KimM4movingaverages_MsgLengthType pMsgLength = KimM4movingaverages_MsgLengthType.Verbose;
//        [NinjaScriptProperty]
//		[Description("Length of the Alert message")]
//		[Category("Alert")]
//		public KimM4movingaverages_MsgLengthType MsgLength
//		{
...
//		private KimM4movingaverages_ColoringBasis pColoringBasis = KimM4movingaverages_ColoringBasis.OnCrossing;
//		[Description("When to color the MA lines?  When the two MA cross, or when the MA lines change their trend direction?")]
//		[Category("Visual")]
//		public KimM4movingaverages_ColoringBasis ColoringBasis
//		{
...
//		private KimM4movingaverages_SymbolType pSymbolTypeInward = KimM4movingaverages_SymbolType.Arrows;
...
//		public KimM4movingaverages_SymbolType SymbolTypeInward
//		{
//			get { return pSymbolTypeInward; }
//			set { pSymbolTypeInward = value; }
//		}

//		private KimM4movingaverages_SymbolType pSymbolTypeOutward = KimM4movingaverages_SymbolType.Triangles;
...
//		public KimM4movingaverages_SymbolType SymbolTypeOutward
//		{
...
//		}
		...
		[NinjaScriptProperty]
		[Category("Parameters")]
		public KimM4movingaverages_DataType MA1DataType
		{
		...
		[NinjaScriptProperty]
		[Category("Parameters")]
		public KimM4movingaverages_type MA1Type
		{
			get { return pMA1Type; }
			set { pMA1Type = value; }
		}
		...
		[Category("Parameters")]
		[NinjaScriptProperty]
		public KimM4movingaverages_DataType MA2DataType
		{
		...
		[NinjaScriptProperty]
		[Category("Parameters")]
		public KimM4movingaverages_type MA2Type
		{
			get { return pMA2Type; }
			set { pMA2Type = value; }
		}
...
[Display(ResourceType = typeof(Custom.Resource), Name = "Marker Down",  GroupName = "Inward Visual")]
	...
	public enum KimM4movingaverages_type
	{
	SMA,
	EMA,
	WMA,
...
public enum KimM4movingaverages_SymbolType {
	Arrows,Triangles,Diamonds,Squares,Dots,None
}
public enum KimM4movingaverages_DataType {
	Open,High,Low,Close,Median,Typical
}
public enum KimM4movingaverages_ColoringBasis {
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private KimM4movingaverages[] cacheKimM4movingaverages;
		public KimM4movingaverages KimM4movingaverages(int period1, int period2, int validZoneSize, int aTRperiod, double aTRmult, bool printZoneSizeToOutputWindow, bool showInwardSignals, bool showOutwardSignals, int vMA1_VolatilityPeriod, int vMA2_VolatilityPeriod, int kAMAfast1, int kAMAslow1, int kAMAfast2, int kAMAslow2, KimM4movingaverages_DataType mA1DataType, KimM4movingaverages_type mA1Type, KimM4movingaverages_DataType mA2DataType, KimM4movingaverages_type mA2Type)
		{
			return KimM4movingaverages(Input, period1, period2, validZoneSize, aTRperiod, aTRmult, printZoneSizeToOutputWindow, showInwardSignals, showOutwardSignals, vMA1_VolatilityPeriod, vMA2_VolatilityPeriod, kAMAfast1, kAMAslow1, kAMAfast2, kAMAslow2, mA1DataType, mA1Type, mA2DataType, mA2Type);
		}

		public KimM4movingaverages KimM4movingaverages(ISeries<double> input, int period1, int period2, int validZoneSize, int aTRperiod, double aTRmult, bool printZoneSizeToOutputWindow, bool showInwardSignals, bool showOutwardSignals, int vMA1_VolatilityPeriod, int vMA2_VolatilityPeriod, int kAMAfast1, int kAMAslow1, int kAMAfast2, int kAMAslow2, KimM4movingaverages_DataType mA1DataType, KimM4movingaverages_type mA1Type, KimM4movingaverages_DataType mA2DataType, KimM4movingaverages_type mA2Type)
		{
			if (cacheKimM4movingaverages != null)
				for (int idx = 0; idx < cacheKimM4movingaverages.Length; idx++)
					if (cacheKimM4movingaverages[idx] != null && cacheKimM4movingaverages[idx].Period1 == period1 && cacheKimM4movingaverages[idx].Period2 == period2 && cacheKimM4movingaverages[idx].ValidZoneSize == validZoneSize && cacheKimM4movingaverages[idx].ATRperiod == aTRperiod && cacheKimM4movingaverages[idx].ATRmult == aTRmult && cacheKimM4movingaverages[idx].PrintZoneSizeToOutputWindow == printZoneSizeToOutputWindow && cacheKimM4movingaverages[idx].ShowInwardSignals == showInwardSignals && cacheKimM4movingaverages[idx].ShowOutwardSignals == showOutwardSignals && cacheKimM4movingaverages[idx].VMA1_VolatilityPeriod == vMA1_VolatilityPeriod && cacheKimM4movingaverages[idx].VMA2_VolatilityPeriod == vMA2_VolatilityPeriod && cacheKimM4movingaverages[idx].KAMAfast1 == kAMAfast1 && cacheKimM4movingaverages[idx].KAMAslow1 == kAMAslow1 && cacheKimM4movingaverages[idx].KAMAfast2 == kAMAfast2 && cacheKimM4movingaverages[idx].KAMAslow2 == kAMAslow2 && cacheKimM4movingaverages[idx].MA1DataType == mA1DataType && cacheKimM4movingaverages[idx].MA1Type == mA1Type && cacheKimM4movingaverages[idx].MA2DataType == mA2DataType && cacheKimM4movingaverages[idx].MA2Type == mA2Type && cacheKimM4movingaverages[idx].EqualsInput(input))
						return cacheKimM4movingaverages[idx];
			return CacheIndicator<KimM4movingaverages>(new KimM4movingaverages(){ Period1 = period1, Period2 = period2, ValidZoneSize = validZoneSize, ATRperiod = aTRperiod, ATRmult = aTRmult, PrintZoneSizeToOutputWindow = printZoneSizeToOutputWindow, ShowInwardSignals = showInwardSignals, ShowOutwardSignals = showOutwardSignals, VMA1_VolatilityPeriod = vMA1_VolatilityPeriod, VMA2_VolatilityPeriod = vMA2_VolatilityPeriod, KAMAfast1 = kAMAfast1, KAMAslow1 = kAMAslow1, KAMAfast2 = kAMAfast2, KAMAslow2 = kAMAslow2, MA1DataType = mA1DataType, MA1Type = mA1Type, MA2DataType = mA2DataType, MA2Type = mA2Type }, input, ref cacheKimM4movingaverages);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.KimM4movingaverages KimM4movingaverages(int period1, int period2, int validZoneSize, int aTRperiod, double aTRmult, bool printZoneSizeToOutputWindow, bool showInwardSignals, bool showOutwardSignals, int vMA1_VolatilityPeriod, int vMA2_VolatilityPeriod, int kAMAfast1, int kAMAslow1, int kAMAfast2, int kAMAslow2, KimM4movingaverages_DataType mA1DataType, KimM4movingaverages_type mA1Type, KimM4movingaverages_DataType mA2DataType, KimM4movingaverages_type mA2Type)
		{
			return indicator.KimM4movingaverages(Input, period1, period2, validZoneSize, aTRperiod, aTRmult, printZoneSizeToOutputWindow, showInwardSignals, showOutwardSignals, vMA1_VolatilityPeriod, vMA2_VolatilityPeriod, kAMAfast1, kAMAslow1, kAMAfast2, kAMAslow2, mA1DataType, mA1Type, mA2DataType, mA2Type);
		}

		public Indicators.KimM4movingaverages KimM4movingaverages(ISeries<double> input , int period1, int period2, int validZoneSize, int aTRperiod, double aTRmult, bool printZoneSizeToOutputWindow, bool showInwardSignals, bool showOutwardSignals, int vMA1_VolatilityPeriod, int vMA2_VolatilityPeriod, int kAMAfast1, int kAMAslow1, int kAMAfast2, int kAMAslow2, KimM4movingaverages_DataType mA1DataType, KimM4movingaverages_type mA1Type, KimM4movingaverages_DataType mA2DataType, KimM4movingaverages_type mA2Type)
		{
			return indicator.KimM4movingaverages(input, period1, period2, validZoneSize, aTRperiod, aTRmult, printZoneSizeToOutputWindow, showInwardSignals, showOutwardSignals, vMA1_VolatilityPeriod, vMA2_VolatilityPeriod, kAMAfast1, kAMAslow1, kAMAfast2, kAMAslow2, mA1DataType, mA1Type, mA2DataType, mA2Type);
		}
	}
