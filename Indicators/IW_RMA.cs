...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private IW_RMA[] cacheIW_RMA;
		public IW_RMA IW_RMA(int period)
		{
			return IW_RMA(Input, period);
		}

		public IW_RMA IW_RMA(ISeries<double> input, int period)
		{
			if (cacheIW_RMA != null)
				for (int idx = 0; idx < cacheIW_RMA.Length; idx++)
					if (cacheIW_RMA[idx] != null && cacheIW_RMA[idx].Period == period && cacheIW_RMA[idx].EqualsInput(input))
						return cacheIW_RMA[idx];
			return CacheIndicator<IW_RMA>(new IW_RMA(){ Period = period }, input, ref cacheIW_RMA);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.IW_RMA IW_RMA(int period)
		{
			return indicator.IW_RMA(Input, period);
		}

		public Indicators.IW_RMA IW_RMA(ISeries<double> input , int period)
		{
			return indicator.IW_RMA(input, period);
		}
	}
