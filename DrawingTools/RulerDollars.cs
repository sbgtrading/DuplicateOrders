...
{
	/// <summary>
	/// Represents an interface that exposes information regarding a RulerDollars IDrawingTool.
	/// </summary>
	public class RulerDollars : DrawingTool
	{
		...
		private ValueUnit						yValueDisplayUnit			= ValueUnit.Currency;
		...
		private void UpdateTextLayout(ChartControl chartControl, ChartPanel chartPanel, ChartScale chartScale)
		{
			...
			double yDiffTicks	= Math.Round(yDiffPrice / AttachedTo.Instrument.MasterInstrument.TickSize,0);
			...
			switch (YValueDisplayUnit)
			{
				...
				case ValueUnit.Currency	: 
					yValueString = string.Format("{0}-tk\n{1}\n{2}", Math.Abs(yDiffTicks), 
						AttachedTo.Instrument.MasterInstrument.InstrumentType == InstrumentType.Forex
						...
						(yDiffPrice / StartAnchor.Price).ToString("0.00%")
			...
			int idx = startIdx;
			long v = chartBars.Bars.GetVolume(idx);
			while(idx < endIdx){
				idx++;
				v = v + chartBars.Bars.GetVolume(idx);
			}
			...
			string text = string.Format("{0}/tick\n{1,-11}{2,-11}\n{3,-11}{4,-11}\n{5,-10}{6,-10}\nv:{7,-10}",
//				AttachedTo.DisplayName, 
//				AttachedTo.Instrument.MasterInstrument.Description.Replace("Futures",string.Empty).Trim(),
				(AttachedTo.Instrument.MasterInstrument.PointValue*AttachedTo.Instrument.MasterInstrument.TickSize).ToString("C").Replace(".00",string.Empty),
	...
	{
		private static RulerDollars RulerDollarsCore(NinjaScriptBase owner, string tag,  bool isAutoScale, int startBarsAgo, DateTime startTime, double startY, 
			int endBarsAgo, DateTime endTime, double endY, int textBarsAgo, DateTime textTime, double textY, bool isGlobal, string templateName)
		{
			...
			RulerDollars ruler = DrawingTool.GetByTagOrNew(owner, typeof(RulerDollars), tag, templateName) as RulerDollars;
		...
		/// <returns></returns>
		public static RulerDollars RulerDollars(NinjaScriptBase owner, string tag, bool isAutoScale, int startBarsAgo, double startY, int endBarsAgo, double endY, int textBarsAgo, double textY)
		{
			return RulerDollarsCore(owner, tag, isAutoScale, startBarsAgo, Core.Globals.MinDate, startY, endBarsAgo, Core.Globals.MinDate, endY, textBarsAgo, Core.Globals.MinDate, textY, false, null);
		}
		...
		/// <param name="textY">The y value of the 3rd anchor point</param>
		/// <returns></returns>
		public static RulerDollars RulerDollars(NinjaScriptBase owner, string tag, bool isAutoScale, DateTime startTime, double startY, DateTime endTime, double endY, DateTime textTime, double textY)
		{
			return RulerDollarsCore(owner, tag, isAutoScale, int.MinValue, startTime, startY, int.MinValue, endTime, endY, int.MinValue, textTime, textY, false, null);
		}
		...
		/// <returns></returns>
		public static RulerDollars RulerDollars(NinjaScriptBase owner, string tag, bool isAutoScale, int startBarsAgo, double startY, int endBarsAgo, double endY, int textBarsAgo, double textY, bool isGlobal, string templateName)
		{
			return RulerDollarsCore(owner, tag, isAutoScale, startBarsAgo, Core.Globals.MinDate, startY, endBarsAgo, Core.Globals.MinDate, endY, textBarsAgo, Core.Globals.MinDate, textY, isGlobal, templateName);
		}
		...
		/// <param name="templateName">The name of the drawing tool template the object will use to determine various visual properties</param>
		/// <returns></returns>
		public static RulerDollars RulerDollars(NinjaScriptBase owner, string tag, bool isAutoScale, DateTime startTime, double startY, DateTime endTime, double endY, DateTime textTime, double textY, bool isGlobal, string templateName)
		{
			return RulerDollarsCore(owner, tag, isAutoScale, int.MinValue, startTime, startY, int.MinValue, endTime, endY, int.MinValue, textTime, textY, isGlobal, templateName);
		}
	}
}
